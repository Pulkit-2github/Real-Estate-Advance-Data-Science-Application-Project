...
group_df = new_df.groupby('sector').mean(['price_per_sqft','price','built_up_area','study room',
                                          'servant room','store room','pooja room','furnishing_type','luxury_score',
...
sns.histplot(new_df[new_df['property_type'] == 'house']['price'], label='house', ax=ax_dist, kde=True)
sns.histplot(new_df[new_df['property_type'] == 'flat']['price'], label='flat', ax=ax_dist, kde=True)
ax_dist.legend()
st.pyplot(fig_dist)
